...
function calculator(num1, operator, num2) {
    ...
    if (operator != '+' && operator != '-' && operator != '*' && operator != "/") {
        result = `${operator} is an unknown operator, I'm aborting.`;
        return result
    }
    // Optional validating addition:
    // else if (isNaN(num1) || isNaN(num2)) {
    //     result = `Either ${num1} or ${num2} is not a number, I'm aborting.`;
    //     return result;
    // }
    else if (num2 === 0) {
        result = `One can't divide by ${num2}, I'm aborting.`;
        return result;
    }
    else {
